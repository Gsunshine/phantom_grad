...
def parse_args():
    ...
    parser.add_argument('--repeat',
                        '-r',
                        help='repeat times for experiment',
                        default=1,
                        type=int)
...
def main(args, logger, final_output_dir, tb_log_dir):
...
if __name__ == '__main__':
    ...
    for steps in steps_list:
        config['TRAIN']['PG_STEPS'] = steps

        best_rec = list()
        for _ in range(args.repeat):
            best_acc = main(args, logger, final_output_dir, tb_log_dir)
