...
from models.mdeq_forward_backward import MDEQWrapper
...
class MDEQModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_channels, dropout=0.0):
        """
...
class MDEQNet(nn.Module):
    ...
    def __init__(self, tau, steps, bn_momentum=0.1):
        """
        ...
        self.fullstage = MDEQModule(self.num_branches, BasicBlock, self.num_blocks, self.num_channels, dropout=self.dropout)

        if self.wnorm:
            self.fullstage._wnorm()

        self.deq = MDEQWrapper(self.fullstage, self.tau, self.pg_steps)
    ...
    def _forward(self, x, train_step=-1, **kwargs):
        ...
        z_list = self.deq(z_list, x_list, 
                threshold=f_thres, train_step=train_step, writer=writer)
