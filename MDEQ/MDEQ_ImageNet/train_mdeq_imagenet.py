...
parser.add_argument('--tau', type=float, default=0.5, help="parameter `tau` in gating strategy")
parser.add_argument('--pg-steps', type=int, default=5, help="Number of steps in Phantom Grad")
...
def main():
    ...
    model = MDEQClsNet(args.tau, args.pg_steps).to(device)
    ...
    '''
    The original performance in the paper was produced without SyncBN (using bs 16 per GPU).
    We added SyncBN when cleaning code. So you may expect slightly better performance.
    '''
...
def train(train_loader, model, optimizer, scheduler, epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        prediction = model(image, train_step=scheduler.iter_counter)
